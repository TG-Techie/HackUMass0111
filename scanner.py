...
class QRFrame(tk.Frame):
    ...
    def __init__(self, controller, bitmap, label_text):
        super().__init__(controller)

        b = tk.Label(self,text = label_text)
        b.pack()
        ...
        rect_size = 512 // bitmap_size

        x_offset = y_offset = (512-(rect_size * bitmap_size))//2
...
class qr_display(tk.Tk):
    ...
    def new_qr_frame(self, qr, label_text):
...
def exchange(username, update_friend, max_msgs_power = 5, switching_time = .05):
    ...
    while should_continue:
        ...
        if (len(found) == num_qrs):
            ...
            #show_canvas(canvas)
            show_canvas(bitmap.string_to_bitmap(qr_end_front), 'end tag')
